...
@Composable
fun ManagerRLRScreen(
    onBack: () -> Unit,
    leaveViewModel: LeaveViewModel,
    user: User?
) {
    LaunchedEffect(Unit) {
        leaveViewModel.getRejectedLeaves(user?.uid!!)
    }
    val leaves by leaveViewModel.rejectedLeaveList.collectAsStateWithLifecycle()
    Column(modifier = Modifier.fillMaxSize()) {
        TopBar(onNavBack = onBack, name = "Rejected Leaves")
    ...
    ElevatedCard(modifier = Modifier
        ...
        Column(modifier = Modifier
            ...
            Row(modifier = Modifier.fillMaxWidth(),
                ...
                Icon(painter = painterResource(R.drawable.rejected), contentDescription = null)
            }
            Text("Raghul.A")
            Spacer(modifier = Modifier.padding(vertical = 8.dp))
